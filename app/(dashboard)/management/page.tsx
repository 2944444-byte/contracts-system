  ...
  async function loadAll() {
    ...
    const [{ data: f }, { data: c }] = await Promise.all([
      supabase.from("management_fees")
        .select("*, contracts(tenants(name), properties(name), management_fee_pct, management_fee_fixed, rent_per_sqm, charged_area, investment_addition)")
      ...
      supabase.from("contracts")
        ...
        .or("management_fee_pct.not.is.null,management_fee_fixed.not.is.null"),
    ]);
    setFees(f ?? []);
    setContracts(c ?? []);
    setLoading(false);
  ...
  async function generateAll() {
    ...
    try {
      ...
      for (const c of contracts) {
        ...
        const feeAmount = c.management_fee_pct
          ? baseRent*(c.management_fee_pct/100)
          : (c.management_fee_fixed ?? 0);
          ...
          <table className="w-full text-right text-sm">
            ...
            <tbody>
              {filtered.map(function(f) {
                ...
                const fixed  = f.contracts?.management_fee_fixed;
                return (
