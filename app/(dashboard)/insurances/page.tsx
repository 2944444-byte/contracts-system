...
export default function InsurancesPage() {
  ...
  async function loadAll() {
    const [{ data: b }, { data: t }, { data: p }, { data: c }, { data: ch }] = await Promise.all([
      ...
      supabase.from("insurances_tenant").select("*, contracts(id, property_id, is_amendment, parent_contract_id, no_tenant_insurance_required, insurance_requirements, tenants(name), properties(name), contract_spaces(area_override,follows_contract_options,spaces(space_name)))").order("end_date"),
      supabase.from("properties").select("id,name").order("name"),
      supabase.from("contracts")
        .select("id, tenant_id, property_id, start_date, end_date, status, is_amendment, parent_contract_id, amendment_date, amendment_number, no_tenant_insurance_required, insurance_requirements, tenants(name), properties(name), contract_spaces(area_override,follows_contract_options,spaces(space_name))")
