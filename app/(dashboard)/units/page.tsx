...
export default function UnitsPage() {
  ...
  async function loadAll() {
    const [{ data: sp }, { data: pr }, { data: c }] = await Promise.all([
      supabase.from("spaces").select("*, properties(name)").order("name"),
  ...
  return (
    <div dir="rtl">
      <div className="mb-6 flex items-center justify-between"><div><h1 className="text-3xl font-bold text-slate-800">יחידות</h1><p className="text-sm text-slate-500 mt-1">{spaces.length} יחידות | {occupied} מושכרות | {vacant} פנויות</p></div><button onClick={openNew} className="rounded-lg bg-blue-700 px-5 py-2.5 font-bold text-white hover:bg-blue-800">+ יחידה</button></div>

      <div className="grid grid-cols-4 gap-3 mb-5">
        {[{label:"מושכרות",value:occupied,pct:spaces.length>0?Math.round(occupied/spaces.length*100):0,color:"text-green-700",bg:"bg-green-50",f:"occupied"},{label:"פנויות",value:vacant,pct:spaces.length>0?Math.round(vacant/spaces.length*100):0,color:"text-blue-700",bg:"bg-blue-50",f:"vacant"},{label:'שטח מוחכר',value:spaces.filter(function(s){return s.status==="occupied";}).reduce(function(s,sp){return s+(sp.area??0);},0)+' מ"ר',pct:totalArea>0?Math.round(spaces.filter(function(s){return s.status==="occupied";}).reduce(function(s,sp){return s+(sp.area??0);},0)/totalArea*100):0,color:"text-slate-700",bg:"bg-white",f:"occupied"},{label:'שטח כולל',value:totalArea+' מ"ר',pct:100,color:"text-slate-500",bg:"bg-white",f:"all"}].map(function(k){
