...
export default function UsersPage() {
  ...
  function openNew() {
    setIsNew(true); setEditId("new");
    setFEmail(""); setFName(""); setFRole("viewer"); setFPassword("");
  }

  function openEdit(u: any) {
    setIsNew(false); setEditId(u.id);
    setFEmail(u.email??""); setFName(u.full_name??""); setFRole(u.role??"viewer"); setFPassword("");
  }
  ...
  async function handleCreate() {
    ...
    try {
      const res = await fetch("/api/admin/create-user", {
        ...
        body: JSON.stringify({ email: fEmail.trim(), password: fPassword, fullName: fName.trim(), role: fRole }),
      });
      const d = await res.json();
      if (!res.ok) throw new Error(d.error ?? "שגיאה");
      setEditId(""); showMsg("✅ משתמש נוצר בהצלחה");
      await loadUsers();
  ...
  async function handleToggleActive(id: string, current: boolean) {
    ...
    showMsg(!current ? "✅ משתמש הופעל" : "✅ משתמש הושבת");
    await loadUsers();
  }

  const roleInfo = function(v: string) {
    return ROLES.find(function(r){return r.v===v;}) ?? ROLES[2];
  };
  ...
  return (
    <div dir="rtl">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">משתמשים</h1>
          <p className="text-sm text-slate-500 mt-1">{users.length} משתמשים רשומים</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        {ROLES.map(function(r) {
          const cnt = users.filter(function(u){return u.role===r.v && u.is_active;}).length;
          return (
            <div key={r.v} className="rounded-xl border border-slate-200 bg-white p-3 text-center">
              <div className="text-2xl mb-0.5">{r.icon}</div>
              ...
              {users.map(function(u) {
                ...
                return (
                  <tr key={u.id} className={"border-t border-slate-100 " + (u.is_active?"hover:bg-slate-50":"opacity-50 bg-slate-50")}>
                    ...
                    <td className="px-4 py-3">
                      <span className={"text-xs px-2 py-0.5 rounded-full font-semibold " + ri.color}>
                        {ri.icon} {ri.l}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1 flex-wrap">
                        {/* שינוי תפקיד */}
                        <select value={u.role} onChange={function(e){handleUpdateRole(u.id,e.target.value);}}
                          className="text-xs border border-slate-200 rounded px-2 py-1 text-slate-600 bg-white">
                          {ROLES.map(function(r){return <option key={r.v} value={r.v}>{r.l}</option>;})}
                        </select>
                        <button onClick={function(){handleToggleActive(u.id,u.is_active);}}
                          className={"text-xs border rounded px-2 py-1 " +
                            (u.is_active?"border-red-100 text-red-500 hover:bg-red-50":"border-green-200 text-green-600 hover:bg-green-50")}>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={function(){setEditId("");}}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md" onClick={function(e){e.stopPropagation();}} dir="rtl">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              ...
              <button onClick={function(){setEditId("");}} className="text-2xl text-slate-400">×</button>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {ROLES.map(function(r) {
                    return (
                      <button key={r.v} type="button" onClick={function(){setFRole(r.v);}}
                        className={"rounded-xl border p-2.5 text-center " +
                          (fRole===r.v?"border-blue-500 bg-blue-50":"border-slate-200 hover:bg-slate-50")}>
                        <div className="text-xl">{r.icon}</div>
                        <div className={"text-xs font-semibold " + (fRole===r.v?"text-blue-700":"text-slate-600")}>{r.l}</div>
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="rounded-xl bg-yellow-50 border border-yellow-200 p-3 text-xs text-yellow-800">
                ⚠️ יצירת משתמש דורשת SUPABASE_SERVICE_ROLE_KEY ב-Vercel
              </div>
              <div className="flex gap-3 pt-2">
                <button onClick={function(){setEditId("");}} className="flex-1 rounded-lg border border-slate-200 py-2.5 text-sm text-slate-600">ביטול</button>
                <button onClick={handleCreate} disabled={saving} className="flex-1 rounded-lg bg-blue-700 py-2.5 text-sm font-bold text-white disabled:opacity-50">
