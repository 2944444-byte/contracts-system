...
export default function ContractsPage() {
  ...
  const stats = {
    ...
    totalRevenue: contracts.filter(c => c.computedStatus === "active").reduce((s, c) => s + (c.rent_per_sqm ?? 0) * (c.charged_area ?? 0) + (c.investment_addition ?? 0), 0),
  };
  ...
  return (
    <div dir="rtl">
      ...
      <div className="mb-4 flex flex-wrap gap-3">
        <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍  חיפוש לפי שוכר או נכס..." className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-800 placeholder-slate-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 flex-1 min-w-48" />
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-800 shadow-sm focus:outline-none">
                ...
                return (
                  <tr key={c.id} onClick={() => setSelected(c)} className="border-t border-slate-50 hover:bg-slate-50 cursor-pointer">
                    ...
                    <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                      <button onClick={() => handleDelete(c.id)} className="text-xs border border-red-100 rounded px-2 py-1 text-red-400 hover:text-red-600">מחיקה</button>
                    </td>
                  </tr>
                );
              })}
      ...
      {selected && (
        <div className="fixed inset-y-0 left-0 w-96 bg-white border-r border-slate-200 shadow-xl z-40 overflow-y-auto" dir="rtl">
          ...
          <div className="p-6 space-y-4">
            ...
            <button onClick={() => handleDelete(selected.id)} className="w-full rounded-lg border border-red-200 py-2 text-sm text-red-500 hover:bg-red-50">🗑️ מחק חוזה</button>
          </div>
        </div>
      )}
    </div>
