...
export default function ContractsPage() {
  ...
  async function loadContracts() {
    const { data } = await supabase.from("contracts")
      .select("*, tenants(name,phone,email,company_name), properties(name,city), contract_spaces(spaces(name,area)), contract_options(id,status,end_date), guarantees(id,status,amount_required,amount_actual)")
      .order("end_date");
    setContracts(data??[]);
    setLoading(false);
