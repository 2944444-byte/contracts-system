...
export default function GroupsPage() {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const payload = { name: fName.trim(), notes: fNotes||null };
      if (isNew) {
  ...
  function groupProperties(groupId: string) {
    return properties.filter(function(p) { return p.group_id===groupId; });
  }

  // KPI
  const totalProps = properties.length;
  const inGroups   = properties.filter(function(p){return p.group_id;}).length;
    ...
    <div dir="rtl">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">{groups.length} קבוצות | {inGroups}/{totalProps} נכסים משויכים</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border-2 border-dashed border-slate-200 bg-white p-12 text-center text-slate-400">
          ...
          <button onClick={openNew} className="mt-3 text-blue-600 hover:underline text-sm">+ קבוצה חדשה</button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {groups.map(function(g) {
            const props = groupProperties(g.id);
            return (
              <div key={g.id} className="rounded-2xl border border-slate-200 bg-white shadow-sm hover:shadow-md transition-shadow">
                <div className="p-5">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex gap-1">
                      ...
                      <button onClick={function(){handleDelete(g.id);}} className="text-xs border border-red-100 rounded px-2 py-1 text-red-400 hover:bg-red-50">🗑</button>
                    </div>
                  </div>
                  ...
                  <div className="rounded-xl bg-slate-50 p-3">
                    ...
                    {props.length === 0 ? (
                      <div className="text-xs text-slate-400">אין נכסים בקבוצה זו</div>
                    ) : (
                      <div className="space-y-1">
                        {props.slice(0,4).map(function(p) {
                          return (
                        ...
                        {props.length > 4 && (
                          <div className="text-xs text-slate-400">+{props.length-4} נוספים...</div>
                        )}
                      </div>
                    )}
                  </div>
                ...
                <div className="border-t border-slate-100 px-5 py-2.5 flex justify-between items-center">
                  ...
                  <button onClick={function(){router.push("/properties");}}
                    className="text-xs text-blue-600 hover:underline">נהל נכסים →</button>
                </div>
              </div>
            );
          })}
          ...
          {properties.filter(function(p){return !p.group_id;}).length > 0 && (
            <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-5">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-2xl">📋</span>
                <h3 className="font-semibold text-slate-600">ללא קבוצה</h3>
              </div>
              <div className="space-y-1">
                {properties.filter(function(p){return !p.group_id;}).map(function(p) {
                  return (
                    <div key={p.id} className="text-xs text-slate-500 flex items-center gap-1">
                      <span>🏢</span><span>{p.name}</span>
                    </div>
                  );
                })}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={function(){setEditingId("");}}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md" onClick={function(e){e.stopPropagation();}} dir="rtl">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-700">שם קבוצה *</label>
                <input type="text" value={fName} onChange={function(e){setFName(e.target.value);}} className={ic} placeholder="מרכז מסחרי / קמפוס / אזור תעשייה" />
              </div>
              <div>
