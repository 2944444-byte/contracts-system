...
export default function LettersPage() {
  ...
  async function loadAll() {
    const [{ data: l }, { data: c }, { data: t }] = await Promise.all([
      ...
      supabase.from("contracts")
        .select("id, tenants(name, contact_name, contact_phone), properties(name, address, city)")
  ...
  function openNew() {
    ...
    setFContractId(""); setFType("notice"); setFSubject(""); setFBody(""); setFTemplateId("");
  }

  function fillTemplate(templateId: string, contractId: string) {
    ...
    if (!tpl || !contract) return;
    let body = tpl.body_template ?? "";
    body = body.replace(/\{\{tenant_name\}\}/g,    contract.tenants?.name    ?? "");
    body = body.replace(/\{\{contact_name\}\}/g,   contract.tenants?.contact_name ?? "");
    body = body.replace(/\{\{property_name\}\}/g,  contract.properties?.name ?? "");
    body = body.replace(/\{\{date\}\}/g,            new Date().toLocaleDateString("he-IL"));
    body = body.replace(/\{\{address\}\}/g,         contract.properties?.address ?? "");
    setFBody(body);
  }
  ...
  const typeInfo = function(v: string) {
    return LETTER_TYPES.find(function(t){return t.v===v;})??LETTER_TYPES[4];
  };
    ...
    <div dir="rtl">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">{letters.length} מכתבים</p>
        </div>
                ...
                return (
                  <tr key={l.id} className="border-t border-slate-100 hover:bg-slate-50">
                    <td className="px-4 py-3">
                      ...
                      {l.body && <div className="text-xs text-slate-400 truncate max-w-xs">{l.body.substring(0,60)}...</div>}
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        <button onClick={function(){setPreview(l);}}
                          className="text-xs border border-slate-200 rounded px-2 py-1 text-slate-600 hover:bg-slate-50">👁 תצוגה</button>
                        <button onClick={function(){handlePrint(l);}}
                          className="text-xs border border-blue-200 rounded px-2 py-1 text-blue-600 hover:bg-blue-50">🖨 הדפס</button>
                        <button onClick={function(){deleteLetter(l.id);}}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={function(e){e.stopPropagation();}} dir="rtl">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="mb-1 block text-xs font-semibold text-slate-700">תבנית</label>
              ...
              <div>
                <label className="mb-2 block text-xs font-semibold text-slate-700">סוג מכתב</label>
                <div className="flex gap-2 flex-wrap">
                  {LETTER_TYPES.map(function(t) {
                    return (
                      <button key={t.v} type="button" onClick={function(){setFType(t.v);}}
                        className={"rounded-xl border px-3 py-1.5 text-xs font-semibold " +
                          (fType===t.v?"border-blue-500 bg-blue-50 text-blue-700":"border-slate-200 text-slate-600")}>
                        {t.icon} {t.l}
                      </button>
                    );
                  })}
              ...
              <div>
                ...
                <textarea value={fBody} onChange={function(e){setFBody(e.target.value);}} rows={10} className={ic + " font-mono text-xs"} />
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={function(e){e.stopPropagation();}} dir="rtl">
            ...
            <div className="p-6">
              <div className="text-xs text-slate-400 mb-4">
                תאריך: {fmtDate(preview.created_at)} | {preview.contracts?.tenants?.name} | {preview.contracts?.properties?.name}
              </div>
              <div className="text-sm text-slate-800 leading-relaxed whitespace-pre-wrap bg-slate-50 rounded-xl p-4 border">
                {preview.body ?? "אין תוכן"}
              </div>
            </div>
          </div>
        </div>
