...
export default function RevenuePage() {
  ...
  async function loadAreaSegments(contractId: string) {
    ...
    const [baseRes, amendsRes] = await Promise.all([
      supabase.from("contracts")
        .select("id, property_id, start_date, charged_area, contract_spaces(area_override,follows_contract_options,space_id, spaces(space_name,area))")
        .eq("id", contractId).single(),
      supabase.from("contracts")
        .select("id, amendment_date, start_date, charged_area, contract_spaces(area_override,follows_contract_options,space_id, spaces(space_name,area))")
  ...
  return (
    <div dir="rtl">
      ...
      {editingId && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onMouseDown={function(e){ if (e.target !== e.currentTarget) return; (closeModal)(); }}>
