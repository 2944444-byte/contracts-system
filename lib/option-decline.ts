...
export async function previewOptionDecline(optionId: string, noticeDate?: Date): Promise<DeclinePreview | null> {
  ...
  const { data: contract } = await supabase.from("contracts")
    .select("id, tenant_id, property_id, vat_type, start_date, end_date, index_base_date, lease_period_value, lease_period_unit, tenants(name), contract_spaces(area_override,follows_contract_options,space_id, spaces(area))")
