...
export async function reconcileAlerts(supabase: SupabaseClient): Promise<{ resolved: number }> {
  ...
  if (contractIds.length > 0) {
    const { data: cs } = await supabase.from("contracts")
      .select("id,end_date,status,min_rent_condition_type,min_rent_condition_met_at,planned_handover_date,actual_handover_date,planned_opening_date,actual_opening_date,works_start_date,works_end_date,grace_months,grace_days,grace_phase2_days,grace_type,grace_ends_on_opening,late_opening_penalty_type,late_opening_penalty_value,late_opening_grace_days,rent_type,rent_per_sqm,min_rent_per_sqm,minimum_rent,charged_area,investment_addition,start_date,contract_options(id,status,is_exercised)").in("id", contractIds);
